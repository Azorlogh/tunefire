...
use druid::{
	...
	Affine, Data, Insets, LinearGradient, UnitPoint, WidgetExt,
};
use tracing::{instrument, trace};

use crate::widget::controllers::OnKey;
...
pub struct FocusableButton<T> {
	label: Label<T>,
	label_size: Size,
}

impl<T: Data> FocusableButton<T> {
	...
	pub fn new(text: impl Into<LabelText<T>>) -> FocusableButton<T> {
		FocusableButton::from_label(Label::new(text))
	}
	...
	/// [`.on_click`]: #method.on_click
	pub fn from_label(label: Label<T>) -> FocusableButton<T> {
		FocusableButton {
			label,
	...
	pub fn dynamic(text: impl Fn(&T, &Env) -> String + 'static) -> Self {
		...
		FocusableButton::new(text)
	}

	/// Provide a closure to be called when this button is clicked.
	pub fn on_click(
		self,
		f: impl Fn(&mut EventCtx, &mut T, &Env) + Clone + 'static,
	) -> ControllerHost<ControllerHost<Self, Click<T>>, OnKey<T>> {
		self.controller(Click::new(f.clone()))
			.controller(OnKey::new(Key::Enter, f))
	}
}

impl<T: Data> Widget<T> for FocusableButton<T> {
	...
	fn event(&mut self, ctx: &mut EventCtx, event: &Event, _data: &mut T, _env: &Env) {
		match event {
			...
			Event::KeyDown(evt) if evt.key == Key::Tab => {
				if evt.mods.shift() {
					ctx.focus_prev();
				} else {
					ctx.focus_next();
	...
	fn paint(&mut self, ctx: &mut PaintCtx, data: &T, env: &Env) {
		...
		let border_color = if (is_hot || ctx.has_focus()) && !ctx.is_disabled() {
